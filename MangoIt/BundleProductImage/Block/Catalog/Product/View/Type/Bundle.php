 ...
 */
namespace MangoIt\BundleProductImage\Block\Catalog\Product\View\Type;
...
class Bundle extends \Magento\Bundle\Block\Catalog\Product\View\Type\Bundle
{
    ...
    public function getSummaryData()
    {
        ...
        $optionArray = array();
        $count = 0 ;
         foreach ($jsonArray['options'] as $key => $value) {
            $count = 0;
            foreach ($value['selections'] as $value1) {
                $optionArray[$key][$count]['small_image'] = $value1['small_image'];
                $optionArray[$key][$count]['name'] = $value1['name'];
                $optionArray[$key][$count]['qty'] = $value1['qty'];
                $optionArray[$key][$count]['unit_price_per_person'] = $value1['unit_price_per_person'];
                $count++;
            }
        }
    ...
    public function getSummaryOptionTitle()
    {
        $optionsArray = $this->getOptions();
        $optionArray = array();
        $count = 0;
        ...
        foreach ($optionsArray as $optionItem) {
            $optionArray[$optionItem->getId()] = $optionItem->getTitle();
            $count++;  
        } 
        return $optionArray;
    }

    public function getProductCustomOptionData()
    {
        $productCustomOptions = $this->getProduct()->getOptions();
        ...
        foreach ($productCustomOptions as $option) {
            $customOptions[$count]['title'] = $option->getTitle();
            foreach ($option->getValues() as $values) {
                $customOptions[$count]['values'][$count1]['option_id'] = $values->getOptionId();
                $customOptions[$count]['values'][$count1]['title'] = $values->getTitle();
                $customOptions[$count]['values'][$count1]['price'] = $values->getPrice();
                $customOptions[$count]['values'][$count1]['price_type'] = $values->getPriceType();
                $count1++;
            }
            $count++;
        }
    ...
    private function getSelectionItemData(Product $product, Product $selection)
    {
        ...
        return [
            ...
            'small_image' => $selection->getSmallImage(),
            'unit_price_per_person' => $selection->getUnitPricePerPerson(),
            'canApplyMsrp' => false,
        ];
    }
    ...
    public function getSummarySelections(Option $option, Product $product)
    {
        $selections = [];
        $selectionCount = count($option->getSelections());
        foreach ($option->getSelections() as $selectionItem) {
            /* @var $selectionItem Product */
            $selectionId = $selectionItem->getSelectionId();
            $selections['selections'][] = $this->getSelectionItemData($product, $selectionItem);
