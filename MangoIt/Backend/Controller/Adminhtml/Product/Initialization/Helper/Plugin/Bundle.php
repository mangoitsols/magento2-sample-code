<?php
/* Overrided file
   Purpose: For removing blank row in bundle option. */

/**
 * Copyright © Mangoit, Inc. All rights reserved.
 * See COPYING.txt for license details.
 */
namespace MangoIt\Backend\Controller\Adminhtml\Product\Initialization\Helper\Plugin;
...
class Bundle extends \Magento\Bundle\Controller\Adminhtml\Product\Initialization\Helper\Plugin\Bundle
{
    ...
    protected function processBundleOptionsData(\Magento\Catalog\Model\Product $product)
    {
        ...
        foreach ($bundleOptionsData as $key => $optionData) {
            if (isset($optionData['delete']) && !empty($optionData['delete'])) {
                continue;
            }
            ...
            foreach ($bundleLinks[$key] as $linkData) {
                /* Code added for skip blank row in bundle option */
                if(isset($linkData['selection_id'])){
                // if (!empty($linkData['delete'])) {
                    if (isset($linkData['delete']) && !empty($linkData['delete'])) {
                    continue;
                }
            ...
            /* Ended code added for skip blank row in bundle option */
        }
