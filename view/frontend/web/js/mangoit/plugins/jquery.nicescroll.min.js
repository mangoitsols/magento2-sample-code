...
this.doc.css("-webkit-backface-visibility", "hidden"); // prevent flickering
}
...
self.rail.drag.ck = (maxh > 0) ? "v" : false; } else if (self.rail.scrollable && !self.railh.scrollable) {
self.rail.drag.ck = (maxw > 0) ? "h" : false; } else {
                                ...
                                if (self.rail.drag.dl) {
                                    ...
                                    if (self.rail.drag.ck == "v") {
                                        if (ay > dz && (ax <= (ay * 0.3))) {
                                            ...
                                            self.rail.drag.dl = "f";
                                            // stop iOS native scrolling (when active javascript has blocked)
...
self.cancelScroll(); //inverted movement detection
                ...
                if (self.opt.bouncescroll == false) {
                    if (y < 0) {
y = 0; } else if (y > self.page.maxh) {
y = self.page.maxh; }
                    if (x < 0) {
x = 0; } else if (x > self.page.maxw) {
x = self.page.maxw; }
                }
...
self.setScrollLeft(px); // fire event onscroll left
                ...
                if (py < 0) {
                  py = 0 ; } else if (py > self.page.maxh) {
                    py = self.page.maxh; }
                if (px < 0) {
                  px = 0; } else if (px > self.page.maxw) {
px = self.page.maxw; }
...
scx = self.newscrollx; }
                    ...
                    if (done == 2) {
                        ...
                        self.scrollrunning = false;
                        if (sc < 0) {
sc = 0; } else if (sc > self.page.maxh) {
sc = self.page.maxh; }
                        if (scx < 0) {
scx = 0; } else if (scx > self.page.maxw) {
scx = self.page.maxw; }
                        if ((scx != self.newscrollx) || (sc != self.newscrolly)) {
self.doScrollPos(scx, sc); } else {
                            if (self.onscrollend) {
                                self.triggerScrollEnd();
                            }
                        }
...
clearAnimationFrame(self.timer); }
        ...
        this.doScrollBy = function (stp, relative) {
            ...
            if (self.bouncescroll) {
                ...
                if (ny < -haf) {
                  ny = -haf; } else if (ny > (self.page.maxh + haf)) {
ny = (self.page.maxh + haf); }
            ...
            if (ny < 0 && py <= 0) {
return self.noticeCursor(); } else if (ny > self.page.maxh && py >= self.page.maxh) {
                self.checkContentSize();
        ...
        this.doScrollLeftBy = function (stp, relative) {
            ...
            if (self.bouncescroll) {
                ...
                if (nx < -haf) {
nx = -haf; } else if (nx > (self.page.maxw + haf)) {
nx = (self.page.maxw + haf); }
            ...
            if (nx < 0 && px <= 0) {
return self.noticeCursor(); } else if (nx > self.page.maxw && px >= self.page.maxw) {
return self.noticeCursor(); }
        ...
        this.doScrollTo = function (pos, relative) {
            ...
            if (ny < 0) {
ny = 0; } else if (ny > self.page.maxh) {
ny = self.page.maxh; }
            self.cursorfreezed = false;
...
self.stop(); }
    ...
    var _scrollTop = jQueryZalw.fn.scrollTop; // preserve original function

    jQueryZalw.cssHooks["pageYOffset"] = {
    ...
    jQueryZalw.fn.scrollTop = function (value) {
    ...
    var _scrollLeft = jQueryZalw.fn.scrollLeft; // preserve original function
    ...
    jQueryZalw.fn.scrollLeft = function (value) {
        if (typeof value == "undefined") {
            var nice = (this[0]) ? $.data(this[0], '__nicescroll') || false : false;
...
fn(el, lst[a]); }
    ...
    jQueryZalw.fn.getNiceScroll = function (index) {
    ...
    jQueryZalw.extend(jQueryZalw.expr[':'], {
        nicescroll: function (a) {
            return ($.data(a, '__nicescroll')) ? true : false;
        }
    });

    jQueryZalw.fn.niceScroll = function (wrapper, opt) {
        ...
        opt = jQueryZalw.extend({}, opt); // cloning
